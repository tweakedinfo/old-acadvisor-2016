...
import info.tweaked.model.plan.{Requires, Prerequisite}
...
case class AssessmentDescription(title:String, description:String, percentage:Int, lo:Seq[Int])

object TeachingUnit {
  implicit def prereq(u:TeachingUnit):Prerequisite = Prerequisite.Contains(u)

  /** An unlisted unit */
  def x(s:String) = TeachingUnit(s, fuzz = Some(Random.nextString(8)))
